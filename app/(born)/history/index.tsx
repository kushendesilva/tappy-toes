...
import { getTotalMl, useFeedingStore } from "../../../store/feedingStore";
...
export default function BornHistoryRoot() {
  ...
  const feedingSeparateSections = useSettingsStore(
    (s) => s.feedingSeparateSections
  );
  ...
  const setFeedingMlIncrement = useSettingsStore(
    (s) => s.setFeedingMlIncrement
  );
  const setFeedingLogAmount = useSettingsStore((s) => s.setFeedingLogAmount);
  const setFeedingSeparateSections = useSettingsStore(
    (s) => s.setFeedingSeparateSections
  );
  ...
  const setFormulaFeedEnabled = useSettingsStore(
    (s) => s.setFormulaFeedEnabled
  );
  ...
  const handleSwitchMode = () => {
    Alert.alert("Switch Mode", "Switch to pregnancy kick tracking mode?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Switch",
        onPress: () => {
  ...
  const handleOpenPrivacyPolicy = async () => {
    await WebBrowser.openBrowserAsync(
      "https://github.com/kushendesilva/tappy-toes/blob/main/PRIVACY_POLICY.md"
    );
  };
  ...
  const getModalTitle = () => {
    if (modalType === "goal") {
      const typeLabel = activeTab === "diaper" ? "Diaper" : "Feeding (ml)";
    ...
    return (
      <ScrollView style={styles.settingsContainer}>
        ...
        <View style={styles.settingRow}>
          ...
          <Switch
            value={breastFeedEnabled}
            onValueChange={setBreastFeedEnabled}
          />
        </View>
        ...
        <View style={styles.settingRow}>
          ...
          <Switch
            value={formulaFeedEnabled}
            onValueChange={setFormulaFeedEnabled}
          />
        </View>
        ...
        <Pressable style={styles.settingBtn} onPress={handleSetMlIncrement}>
          <Text style={styles.settingBtnText}>
            ML Increment: {feedingMlIncrement}ml
          </Text>
        </Pressable>
        ...
        <View style={styles.settingRow}>
          ...
          <Switch
            value={feedingLogAmount}
            onValueChange={setFeedingLogAmount}
          />
        </View>
        ...
        <View style={styles.settingRow}>
          ...
          <Switch
            value={feedingSeparateSections}
            onValueChange={setFeedingSeparateSections}
          />
        </View>
  ...
  const renderHistoryContent = () => {
    ...
    const typeLabel =
      activeTab === "diaper"
        ...
        ? "Feeding"
        : "Medicine";
    const showGoal = activeTab !== "medicine";
    ...
    return (
      <>
        ...
        <Pressable
          style={[
            styles.resetAllBtn,
            days.length === 0 && styles.resetAllDisabled,
          ]}
        ...
        {showGoal && (
          <Pressable style={styles.goalBtn} onPress={handleSetGoal}>
            ...
            <Text style={styles.goalBtnText}>
              Daily Goal:{" "}
              {activeTab === "feeding" ? `${currentGoal} ml` : currentGoal}
            </Text>
          </Pressable>
        )}
        ...
        <FlatList
                  ...
                  <TouchableOpacity style={styles.row}>
                    ...
                    <View style={styles.rowInfoContainer}>
                      <View style={styles.medicineStatsRow}>
                        {info.taken > 0 && (
                          <Text style={styles.takenText}>✓{info.taken} </Text>
                        )}
                        {info.missed > 0 && (
                          <Text style={styles.missedText}>✗{info.missed} </Text>
                        )}
                        {info.snoozed > 0 && (
          ...
          contentContainerStyle={
            days.length === 0 ? { flexGrow: 1 } : undefined
          }
        />
      ...
      <View style={styles.tabContainer}>
        ...
        >
          <Text
            style={[
              styles.tabText,
              activeTab === "diaper" && styles.activeTabText,
            ]}
          >
            Diaper
          </Text>
...
const styles = StyleSheet.create({
  ...
  activeTab: { backgroundColor: "#4e6af3" },
  ...
  resetAllDisabled: { opacity: 0.4 },
  resetAllText: { color: "#fff", fontSize: 15, fontWeight: "600" },
  goalBtn: {
    flexDirection: "row",
    alignItems: "center",
  ...
  goalBtnText: { color: "#4e6af3", fontSize: 14, fontWeight: "600" },
  row: {
    flexDirection: "row",
    paddingVertical: 14,
  ...
  rowInfoContainer: { flexDirection: "row", alignItems: "center", gap: 12 },
  rowInfoText: { fontSize: 14, color: "#666" },
  medicineStatsRow: { flexDirection: "row", alignItems: "center" },
  takenText: { fontSize: 14, color: "#1B5E20" },
  missedText: { fontSize: 14, color: "#B71C1C" },
  snoozedText: { fontSize: 14, color: "#E65100" },
  sep: { height: 10 },
  ...
  switchModeText: { color: "#4e6af3", fontSize: 15, fontWeight: "600" },
  ...
  settingBtnIcon: { marginRight: 8 },
  settingBtnText: { color: "#4e6af3", fontSize: 16, fontWeight: "600" },
  aboutContainer: {
    backgroundColor: "#f4f6fa",
    padding: 16,
    borderRadius: 10,
  ...
  aboutVersion: { fontSize: 12, color: "#666", marginBottom: 8 },
  aboutDeveloper: { fontSize: 12, color: "#4e6af3", fontWeight: "500" },
