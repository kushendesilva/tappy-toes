...
import { EMPTY_ARRAY, formatTime, getSummary, usePeeStore } from '../store/peeStore';

export const PeeTodaySummary = () => {
  const dayKey = useTodayKey();
  const pees = usePeeStore(s => s.data[dayKey] ?? EMPTY_ARRAY);
  const summary = getSummary(pees);
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>Today&apos;s Pee</Text>
